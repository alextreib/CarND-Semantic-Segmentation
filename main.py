...
def load_vgg(sess, vgg_path):
    ...
    # Getting the graph for the layers
    graph = tf.get_default_graph()
    inputLayer = graph.get_tensor_by_name(vgg_input_tensor_name)
    keepProb = graph.get_tensor_by_name(vgg_keep_prob_tensor_name)
    layer3 = graph.get_tensor_by_name(vgg_layer3_out_tensor_name)
    layer4 = graph.get_tensor_by_name(vgg_layer4_out_tensor_name)
    layer7 = graph.get_tensor_by_name(vgg_layer7_out_tensor_name)

    return inputLayer, keepProb, layer3, layer4, layer7
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    """
    # Getting logits and correct_label by reducing the dimension (see classroom classification&loss)
    logits = tf.reshape(nn_last_layer, (-1, num_classes))
    correct_label = tf.reshape(correct_label, (-1, num_classes))
    # Define cross_entropy (what to minimize).
    cross_entropy_loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=correct_label))
    # Adam optimizer chosen (worked very good in last project)
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
    # Training operation for optimizer
